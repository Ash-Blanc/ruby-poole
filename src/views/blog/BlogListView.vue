...
import { blogPosts, getCategories, getFeaturedPosts } from '@/components/data/blogPosts'
...
<template>
    <SeoHead :title="currentCategory ? `${currentCategory} Articles` : 'Blog & Market Insights'"
        ...
        <section class="py-16 bg-gradient-to-br from-base-100 via-base-200/50 to-primary/5">
            <div class="container-custom">
                ...
                <div class="flex flex-wrap justify-center gap-2 mt-8">
                    <RouterLink to="/blog" class="btn btn-sm" :class="!currentCategory ? 'btn-primary' : 'btn-ghost'">
                        All Posts
                    </RouterLink>
                    <RouterLink v-for="cat in categories" :key="cat" :to="`/blog?category=${encodeURIComponent(cat)}`"
                        class="btn btn-sm" :class="currentCategory === cat ? 'btn-primary' : 'btn-ghost'">
                        {{ cat }}
                    </RouterLink>
                </div>
            </div>
                ...
                <div class="grid lg:grid-cols-3 gap-12">
                    ...
                    <div class="lg:col-span-2">
                        <div v-if="regularPosts.length > 0" class="grid sm:grid-cols-2 gap-6">
                            <BlogCard v-for="post in regularPosts" :key="post.slug" :post="post" />
                        </div>
