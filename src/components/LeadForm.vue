    ...
    <form @submit.prevent="handleSubmit" :class="[
        ...
        <template v-else>
            <div class="form-control">
                <input v-model="form.name" type="text" placeholder="Your Name" class="input input-bordered w-full"
                    :class="{ 'input-sm': variant === 'compact' }" required />
            </div>

            <div :class="showPhone ? 'grid sm:grid-cols-2 gap-3' : ''">
                <div class="form-control">
                    <input v-model="form.email" type="email" placeholder="Email Address"
                        class="input input-bordered w-full" :class="{ 'input-sm': variant === 'compact' }" required />
                </div>

                <div v-if="showPhone" class="form-control">
                    <input v-model="form.phone" type="tel" placeholder="Phone (optional)"
                        class="input input-bordered w-full" :class="{ 'input-sm': variant === 'compact' }" />
                </div>
            </div>

            <div class="form-control">
                <select v-model="form.timeline" class="select select-bordered w-full"
                    :class="{ 'select-sm': variant === 'compact' }" required>
            ...
            <button type="submit" class="btn btn-primary w-full gap-2" :class="{
