...
<template>
    <!-- Featured Variant -->
    <RouterLink v-if="variant === 'featured'" :to="`/blog/${post.slug}`"
        ...
        <div class="absolute bottom-0 left-0 right-0 p-6 md:p-10">
            ...
            <h2
                class="text-2xl md:text-4xl font-display text-neutral-content mb-3 group-hover:text-primary transition-colors">
                {{ post.title }}
            </h2>
    ...
    <!-- Compact Variant -->
    <RouterLink v-else-if="variant === 'compact'" :to="`/blog/${post.slug}`" class="flex gap-4 group">
    ...
    <!-- Default Variant -->
    <RouterLink v-else :to="`/blog/${post.slug}`"
